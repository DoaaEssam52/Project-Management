...
export class ForgetPasswordComponent {
  ...
  ngOnInit(): void {
    this._auth.authTitle.next('Forget Password');
    this._auth.authBackgroundImage.next('assets/images/bg-1.jpeg');
  }
  ...
  Verify(): void {
    ...
    if (this.forgetPassForm.valid) {
      this._auth.checkEmail(this.forgetPassForm.value as string).subscribe({
        next: () => {
